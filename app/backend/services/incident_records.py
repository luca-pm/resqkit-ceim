...
from models.incident_records import Incident_records

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Incident_recordsService:
    """Service layer for Incident_records operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Incident_records]:
        """Create a new incident_records"""
        try:
            obj = Incident_records(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created incident_records with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating incident_records: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Incident_records]:
        """Get incident_records by ID"""
        try:
            query = select(Incident_records).where(Incident_records.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching incident_records {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of incident_recordss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Incident_records, field):
                        column = getattr(Incident_records, field)
            ...
            stmt = select(Incident_records, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Incident_records, field_name):
                        stmt = stmt.order_by(getattr(Incident_records, field_name).desc())
                else:
                    if hasattr(Incident_records, sort):
                        stmt = stmt.order_by(getattr(Incident_records, sort))
            else:
                stmt = stmt.order_by(Incident_records.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Incident_records)
                for condition in conditions:
            ...
            logger.error(f"Error fetching incident_records list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Incident_records]:
        """Update incident_records"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Incident_records {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated incident_records {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating incident_records {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete incident_records"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Incident_records {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted incident_records {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting incident_records {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Incident_records]:
        """Get incident_records by any field"""
        try:
            if not hasattr(Incident_records, field_name):
                raise ValueError(f"Field {field_name} does not exist on Incident_records")
            column = getattr(Incident_records, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Incident_records).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching incident_records by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Incident_records]:
        """Get list of incident_recordss filtered by field"""
        try:
            if not hasattr(Incident_records, field_name):
                raise ValueError(f"Field {field_name} does not exist on Incident_records")
            column = getattr(Incident_records, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Incident_records)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Incident_records.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching incident_recordss by {field_name}: {str(e)}")
            raise