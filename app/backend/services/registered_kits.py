...
from models.registered_kits import Registered_kits

logger = logging.getLogger(__name__)


# ------------------ Service Layer ------------------
class Registered_kitsService:
    """Service layer for Registered_kits operations"""
    ...
    async def create(self, data: Dict[str, Any]) -> Optional[Registered_kits]:
        """Create a new registered_kits"""
        try:
            obj = Registered_kits(**data)
            self.db.add(obj)
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Created registered_kits with id: {obj.id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating registered_kits: {str(e)}")
            raise

    async def get_by_id(self, obj_id: int) -> Optional[Registered_kits]:
        """Get registered_kits by ID"""
        try:
            query = select(Registered_kits).where(Registered_kits.id == obj_id)
            result = await self.db.execute(query)
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching registered_kits {obj_id}: {str(e)}")
            raise
    ...
    async def get_list(
        ...
        """Get paginated list of registered_kitss"""
        try:
            ...
            if query_dict:
                for field, value in query_dict.items():
                    if hasattr(Registered_kits, field):
                        column = getattr(Registered_kits, field)
            ...
            stmt = select(Registered_kits, func.count().over().label("total_count"))
            ...
            if sort:
                if sort.startswith('-'):
                    field_name = sort[1:]
                    if hasattr(Registered_kits, field_name):
                        stmt = stmt.order_by(getattr(Registered_kits, field_name).desc())
                else:
                    if hasattr(Registered_kits, sort):
                        stmt = stmt.order_by(getattr(Registered_kits, sort))
            else:
                stmt = stmt.order_by(Registered_kits.id.desc())
            ...
            if rows:
                ...
                items = []
                count_stmt = select(func.count()).select_from(Registered_kits)
                for condition in conditions:
            ...
            logger.error(f"Error fetching registered_kits list: {str(e)}")
            raise

    async def update(self, obj_id: int, update_data: Dict[str, Any]) -> Optional[Registered_kits]:
        """Update registered_kits"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Registered_kits {obj_id} not found for update")
                return None
            ...
            await self.db.commit()
            await self.db.refresh(obj)
            logger.info(f"Updated registered_kits {obj_id}")
            return obj
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error updating registered_kits {obj_id}: {str(e)}")
            raise

    async def delete(self, obj_id: int) -> bool:
        """Delete registered_kits"""
        try:
            obj = await self.get_by_id(obj_id)
            if not obj:
                logger.warning(f"Registered_kits {obj_id} not found for deletion")
                return False
            await self.db.delete(obj)
            await self.db.commit()
            logger.info(f"Deleted registered_kits {obj_id}")
            return True
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error deleting registered_kits {obj_id}: {str(e)}")
            raise

    async def get_by_field(self, field_name: str, field_value: Any) -> Optional[Registered_kits]:
        """Get registered_kits by any field"""
        try:
            if not hasattr(Registered_kits, field_name):
                raise ValueError(f"Field {field_name} does not exist on Registered_kits")
            column = getattr(Registered_kits, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Registered_kits).where(column == field_value)
            )
            return result.scalar_one_or_none()
        except Exception as e:
            logger.error(f"Error fetching registered_kits by {field_name}: {str(e)}")
            raise

    async def list_by_field(
        self, field_name: str, field_value: Any, skip: int = 0, limit: int = 20
    ) -> List[Registered_kits]:
        """Get list of registered_kitss filtered by field"""
        try:
            if not hasattr(Registered_kits, field_name):
                raise ValueError(f"Field {field_name} does not exist on Registered_kits")
            column = getattr(Registered_kits, field_name)
            field_value = self._coerce_field_value(column, field_value, field_name)
            result = await self.db.execute(
                select(Registered_kits)
                .where(column == field_value)
                .offset(skip)
                .limit(limit)
                .order_by(Registered_kits.id.desc())
            )
            return result.scalars().all()
        except Exception as e:
            logger.error(f"Error fetching registered_kitss by {field_name}: {str(e)}")
            raise