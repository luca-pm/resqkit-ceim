...
# Auth uses a bearer token (Authorization header), never cookies, so the API
# does not need CORS "credentials" mode. Only the configured local frontend
# origin is allowed to call this API; there is no cloud proxy or preview-domain
# fleet to support anymore, so the origin list is explicit rather than a
# wildcard.
app.add_middleware(
    CORSMiddleware,
