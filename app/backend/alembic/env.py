...
# alembic.ini deliberately ships with an empty sqlalchemy.url so the same
# DATABASE_URL used by the running app (from .env / the environment) is the
# single source of truth for migrations too.
config.set_main_option("sqlalchemy.url", db_manager._normalize_async_database_url(settings.database_url))
...
def alembic_include_object(object, name, type_, reflected, compare_to):
    ...
    if type_ == "table" and name in ["users", "sessions"]:
        return False
    return True
