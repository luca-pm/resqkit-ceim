/**
 * Settings — Real data mode + the institutional actions trace.
 *
 ...
 * They are simulated on-device by default; see lib/institutionalActions.ts.
 */
    ...
    <div className="space-y-5">
      ...
      <Card className={settings.realDataMode ? 'border-destructive/50' : undefined}>
        ...
        <CardContent className="space-y-3">
          <div className="flex items-center justify-between gap-4 rounded-md border border-border p-3">
            <div>
              <p className="text-sm font-medium">
                {settings.realDataMode ? 'Real data mode is ON' : 'Simulation mode (default)'}
              </p>
              <p className="mt-1 text-xs text-muted-foreground">
                {settings.realDataMode
                  ? 'Institutional actions now call this app’s backend for real — creating a session, and sending it your context, called-112 status, triage answers, and location if captured. It still never transmits to real emergency infrastructure (see NG protocol note below).'
                  : 'Institutional actions (NG112 connection, PVR, the transcript websocket, NG protocol build) are faked on-device only. Nothing is sent anywhere.'}
              </p>
            </div>
            <Switch
              ...
              onCheckedChange={(checked) => {
                ...
                toast.success(checked ? 'Real data mode enabled.' : 'Back to simulation mode.');
              }}
          ...
          <p className="flex items-start gap-2 text-xs text-muted-foreground">
            ...
            Real or simulated, ResQKit never contacts 112 itself. The NG protocol payload builder is
            proof-of-concept only — it is not an authorized channel to Romanian emergency services
            (STS/ANCOM). The only real 112 channel in this app is the dialer button.
          </p>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Every institutional action fired, most recent first, tagged simulated or real.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
                ...
                <div key={entry.id} className="rounded-md border border-border p-2.5 text-sm">
                  <div className="flex flex-wrap items-center gap-1.5">
                    <Badge variant={entry.mode === 'real' ? 'destructive' : 'secondary'}>
                      {entry.mode === 'real' ? 'REAL' : 'SIMULATED'}
                    </Badge>
