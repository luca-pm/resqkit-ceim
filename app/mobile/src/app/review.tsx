  ...
  const finishIncident = () => {
    ...
    toast.success(
      settings.retention === 'session'
        ...
        : `Incident closed. Kept on this device for ${RETENTION_WINDOW[settings.retention]}, then deleted automatically.`,
    );
    router.replace('/');
  };
    ...
    <ScrollView className="flex-1 bg-background" contentContainerClassName="gap-5 p-4 pb-10">
      ...
      {incident ? (
        <>
          ...
          <Button size="lg" variant="destructive" onPress={finishIncident}>
